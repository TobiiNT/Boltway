...
| `mcp-tool-challenge-2026-08-25.md` | why a per-tool refusal cannot carry an authorization challenge on either channel: the field C-24 named is a sponsored draft in no schema, and the HTTP challenge is unreachable because the event stream is open before a tool filter runs. Read before designing a per-tool refusal |
