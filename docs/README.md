...
| [`decisions/mcp-per-tool-authorization-2026-08.md`](decisions/mcp-per-tool-authorization-2026-08.md) | Anyone gating an individual MCP tool on a scope, declaring one on an MCP route, or about to hand-roll a step-up challenge inside a connector | Nothing. §2.1 and §2.2 are what a consumer is exposed to today - one instructs clients to ask for too little, the other fails open on a malformed claim - and §3 is the boundary that outlives every item | decision record |
